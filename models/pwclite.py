...
class PWCLite(nn.Module):
    ...
    def forward(self, x, with_bk=False):
        ...
        imgs = [x[:, 3 * i: 3 * i + 3] for i in range(int(n_frames))] # 2x of [1, 3, 384, 640])

        x = [self.feature_pyramid_extractor(img) + [img] for img in imgs] 

        # print(len(x))
        # print(len(x[0])) # 2
        # print(len(x[1])) # 7 each 
        # print(x[0][6].shape) # [1, 192, 6, 10] # [1, 128, 12, 20] # [1, 96, 24, 40] # [1, 64, 48, 80] # [1, 32, 96, 160] # [1, 16, 192, 320] # [1, 3, 384, 640]
        # print(with_bk)
        # stop
