...
make -j4
cp pyevaluatedepth_lib.cpython-36m-x86_64-linux-gnu.so ../pyevaluatedepth_lib.so
cd ..; cd ..
cd perception_lib
rm -rf build; mkdir build; cd build;
cmake -DCMAKE_BUILD_TYPE=Release ..
make -j4
cp pyperception_lib.cpython-36m-x86_64-linux-gnu.so ../pyperception_lib.so
...
cmake -DCMAKE_BUILD_TYPE=Release ..
make -j4
cp utils_lib.cpython-36m-x86_64-linux-gnu.so ../utils_lib.so
cd ..; cd ..
